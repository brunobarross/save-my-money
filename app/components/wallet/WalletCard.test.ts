...
describe("WalletCard", () => {
  ...
  it("should exist delete button if canDelete is true", async () => {
    ...
    expect(
      component.find('[data-testid="wallet-card-delete-button"]').exists()
    ).toBe(true);
  });
  ...
  it("should show error toast on failed", async () => {
    ...
    mockMutateAsync.mockRejectedValueOnce(
      new Error("Failed on delete this wallet")
    );
    const mockCallback = mockOverlayCreate.mock.lastCall[1].props.onAccept;
    await mockCallback();
